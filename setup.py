...
try:
    ...
    from prompt_toolkit.layout.containers import HSplit, Window, VSplit, DynamicContainer, WindowAlign
...
def install_banner():
    ...
    if config.get("compact_mode_enabled"):
        command_parts_for_shell.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "compact_mode.py"))
    else:
...
def configure_banner():
    ...
    # Compact Mode
    print(f"\nCompact mode currently: {'Enabled' if config.get('compact_mode_enabled') else 'Disabled'}")
    if get_yes_no_input("Toggle compact mode?"):
        config["compact_mode_enabled"] = not config.get("compact_mode_enabled", False)
